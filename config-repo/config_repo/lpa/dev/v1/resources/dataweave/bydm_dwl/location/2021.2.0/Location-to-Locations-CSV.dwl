...
---
Locations : {(payload filter $.documentActionCode != 'DELETE' map (value) -> {
			Record: {
				...
				OpenDate: null,
				FinalDate: null,
				ActiveFrom: if ( value.'basicLocation.status.effectiveFromDate' != null and value.'basicLocation.status.effectiveFromDate' != "" ) (value.'basicLocation.status.effectiveFromDate' replace "Z" with "")
		          else defaultFromDate,
				ActiveUpTo: if ( value.'basicLocation.status.effectiveUpToDate' != null and value.'basicLocation.status.effectiveUpToDate' != "" ) validateDate(value.'basicLocation.status.effectiveUpToDate' replace "Z" with "")
		          else "9999-12-31"
		}
})}