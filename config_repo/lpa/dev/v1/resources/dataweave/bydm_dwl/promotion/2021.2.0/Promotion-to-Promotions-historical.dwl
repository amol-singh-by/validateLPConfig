...
Promotions: {
	(payload[vars.bulkType] map (promo, index) -> {
		...
		Record: {
			...
			PromotionFrom: if ( !isEmpty(eligibilityInfo.effectiveFromDate)) eligibilityInfo.effectiveFromDate
	          else "",
			PromotionUpTo: if ( !isEmpty(eligibilityInfo.effectiveUpToDate) ) eligibilityInfo.effectiveUpToDate
	          else "",
