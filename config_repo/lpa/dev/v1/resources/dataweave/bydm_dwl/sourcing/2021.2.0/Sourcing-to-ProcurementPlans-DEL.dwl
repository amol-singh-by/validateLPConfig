...
---
ProcurementPlans: {
	(payload[vars.bulkType] filter (getOperationType($.documentActionCode) == "Record") map ((item, index) -> {
		DeleteRecord: {
			...
			"CanBeOrderedFrom": if ( item.sourcingDetails.effectiveFromDate != null and item.sourcingDetails.effectiveFromDate != "") item.sourcingDetails.effectiveFromDate else defaultFromDate,
