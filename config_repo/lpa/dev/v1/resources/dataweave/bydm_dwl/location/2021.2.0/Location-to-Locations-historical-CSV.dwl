...
Locations : {(payload filter $.documentActionCode != 'DELETE' map (value) -> {
			Record: {
				...
				ActiveUpTo: if ( value.'basicLocation.status.effectiveUpToDate' != null and value.'basicLocation.status.effectiveUpToDate' != "" ) value.'basicLocation.status.effectiveUpToDate' replace "Z" with ""
		          else "9999-12-31"
		}
})}