...
---
PurchaseGroups : {(payload[vars.bulkType] filter($.documentActionCode != "DELETE") map ((og , index) -> {
	"Record": {
		...
		("UniqueProductPerPallet": og.orderGroupParameters.isSingleItemPallet) if (!isEmpty(og.orderGroupParameters.isSingleItemPallet)),
		"TransportationMinRule": if ( !isEmpty(og.orderGroupParameters.areAllTransportationMinimumRulesRequired) and og.orderGroupParameters.areAllTransportationMinimumRulesRequired ) "ALL" else "ANY",
		"VendorMinRule": if ( !isEmpty(og.orderGroupParameters.areAllSupplierMinimumRulesRequired) and og.orderGroupParameters.areAllSupplierMinimumRulesRequired ) "ALL" else "ANY"
	}
}))}
