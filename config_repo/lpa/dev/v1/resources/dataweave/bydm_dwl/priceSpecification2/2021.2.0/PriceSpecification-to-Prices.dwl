...
Prices: {
	(payload[vars.bulkType] map () -> {
        ...
        if ( getOperationType($) == "DeleteAllRecords" ) {
			...
			ActiveFrom: if ($.priceEffectiveFromDate != null and $.priceEffectiveFromDate != "") $.priceEffectiveFromDate
              else "",
			ActiveUpTo: if ($.priceEffectiveUpToDate != null and $.priceEffectiveUpToDate != "") validateDate($.priceEffectiveUpToDate)
              else ""
		}
