...
---
Prices: {
	(payload filter (getOperationType($) == 'Record') map (item, index) -> 
{
		"DeleteRecord": {
			...
			ActiveUpTo: if ( item.priceEffectiveUpToDate != null and item.priceEffectiveUpToDate != "") validateDate(item.priceEffectiveUpToDate) else ""
		}
	})
}